...
echo "Future Crash + LOOK 6.6.3 · MOVIE NIGHT"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.6.3" ]] || { echo "BUNDLE ERROR: expected release 6.6.3, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · MOVIE NIGHT"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.6.3 · MOVIE NIGHT with MOVIE NIGHT terminal editing, shared ONE BRAIN cognition, LIVING MIND memory, Fabric endpoint management, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
