...
echo "Future Crash + LOOK 4.4.0 · Fabric Integration"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "4.4.0" ]] || { echo "BUNDLE ERROR: expected release 4.4.0, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 4.4.0 and Signal Window 0.8.0"
