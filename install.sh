...
echo "Future Crash + LOOK 6.6.5 · PICTURE LOCK"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.6.5" ]] || { echo "BUNDLE ERROR: expected release 6.6.5, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · PICTURE LOCK"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.6.5 · PICTURE LOCK with PICTURE LOCK terminal editing, shared ONE BRAIN cognition, LIVING MIND memory, Fabric endpoint management, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
