...
echo "Future Crash + LOOK 4.5.0 · Fabric Integration"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "4.5.0" ]] || { echo "BUNDLE ERROR: expected release 4.5.0, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 4.5.0 and Signal Window 0.9.0"
