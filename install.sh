...
echo "Future Crash + LOOK 5.2.12 · Interactive Reliability"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.12" ]] || { echo "BUNDLE ERROR: expected release 5.2.12, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.12, Interactive Reliability, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
