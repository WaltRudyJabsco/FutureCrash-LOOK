...
echo "Future Crash + LOOK 5.1.1 · Signal Gets the Signal"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.1.1" ]] || { echo "BUNDLE ERROR: expected release 5.1.1, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.1.1, Fabric dashboard, and Signal Window 1.1.1"
