...
echo "Future Crash + LOOK 6.4.2 · Clean Paper"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.4.2" ]] || { echo "BUNDLE ERROR: expected release 6.4.2, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.4.2 · Clean Paper with Fabric-wide endpoint management, Tailcat direct TLS transport, enforced Fabric peer authorization, accountless browser endpoint pairing, Fabric SearXNG discovery/search, Decision Plane, Content Search, Media, Artifacts, Memory, and Signal Window 1.10.5"
