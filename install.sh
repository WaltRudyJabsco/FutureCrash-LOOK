...
echo "Future Crash + LOOK 6.6.4 · SHOWTIME"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.6.4" ]] || { echo "BUNDLE ERROR: expected release 6.6.4, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · SHOWTIME"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.6.4 · SHOWTIME with SHOWTIME terminal editing, shared ONE BRAIN cognition, LIVING MIND memory, Fabric endpoint management, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
