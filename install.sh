...
echo "Future Crash + LOOK 6.7.5 · OPEN LINE"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.7.5" ]] || { echo "BUNDLE ERROR: expected release 6.7.5, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · OPEN LINE"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.7.5 · OPEN LINE with signed Fabric rendezvous, Tailcat direct transport, ONE BRAIN cognition, LIVING MIND memory, browser endpoints, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
