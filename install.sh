...
echo "Future Crash + LOOK 5.2.3 · Dash Polish"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.3" ]] || { echo "BUNDLE ERROR: expected release 5.2.3, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.3, Dash Polish telemetry, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
