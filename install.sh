...
echo "Future Crash + LOOK 5.2.17 · Fabric Media Catalog"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.17" ]] || { echo "BUNDLE ERROR: expected release 5.2.17, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.17, Fabric Media Catalog, Streaming Artifacts, Shared Fabric UI Model, Vision Artifacts, Fabric Memory, and Signal Window 1.1.2"
