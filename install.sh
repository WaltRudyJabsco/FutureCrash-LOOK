...
echo "Future Crash + LOOK 5.2.9 · Ollama Command Dispatch Fix"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.9" ]] || { echo "BUNDLE ERROR: expected release 5.2.9, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.9, Ollama Command Dispatch Fix, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
