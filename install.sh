...
echo "Future Crash + LOOK 4.6.5 · Streaming Fabric"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "4.6.5" ]] || { echo "BUNDLE ERROR: expected release 4.6.5, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 4.6.5 and Signal Window 0.9.0"
