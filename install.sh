...
echo "Future Crash + LOOK 6.7.2 · STAY AWAKE"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.7.2" ]] || { echo "BUNDLE ERROR: expected release 6.7.2, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · STAY AWAKE"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.7.2 · STAY AWAKE with signed Fabric rendezvous, Tailcat direct transport, ONE BRAIN cognition, LIVING MIND memory, browser endpoints, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
