...
echo "Future Crash + LOOK 6.8.1 · MODEL GROUND TRUTH"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.8.1" ]] || { echo "BUNDLE ERROR: expected release 6.8.1, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.8.1 · MODEL GROUND TRUTH with signed Fabric rendezvous, Tailcat direct transport, ONE BRAIN cognition, LIVING MIND memory, browser endpoints, SearXNG, Media, Artifacts, and Signal Window 1.11.0"
