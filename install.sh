...
echo "Future Crash + LOOK 5.2.6 · Adaptive Dash Geometry"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.6" ]] || { echo "BUNDLE ERROR: expected release 5.2.6, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.6, Adaptive Dash Geometry, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
