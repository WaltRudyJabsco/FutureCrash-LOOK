...
echo "Future Crash + LOOK 4.7.0 · Signal Native LO"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "4.7.0" ]] || { echo "BUNDLE ERROR: expected release 4.7.0, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 4.7.0 and Signal Window 1.0.0"
