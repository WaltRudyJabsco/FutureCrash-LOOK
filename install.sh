...
echo "Future Crash + LOOK 5.4.8 · Media Output State Repair"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.4.8" ]] || { echo "BUNDLE ERROR: expected release 5.4.8, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.4.8, canonical Decision Plane, optional OpenJev worker, Fabric Media Catalog, Streaming Artifacts, Fabric Memory, and Signal Window 1.6.2"
