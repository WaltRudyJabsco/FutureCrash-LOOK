...
echo "Future Crash + LOOK 5.2.10 · Isolated Model Qualification"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.10" ]] || { echo "BUNDLE ERROR: expected release 5.2.10, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.10, Isolated Model Qualification, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
