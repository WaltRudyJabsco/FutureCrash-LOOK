...
echo "Future Crash + LOOK 5.4.0 · Canonical Decision Worker"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.4.0" ]] || { echo "BUNDLE ERROR: expected release 5.4.0, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.4.0, canonical Decision Plane, optional OpenJev worker, Fabric Media Catalog, Streaming Artifacts, Fabric Memory, and Signal Window 1.2.0"
...
# Keep the optional Local Labs server controller in lockstep when it is already installed.
# Future Crash does not force-install the controller on machines that do not use it.
if [[ -f "$HOME/.local/share/local-labs-host/server.py" ]]; then
  install -m 0755 "$ROOT/local-labs-host/server.py" "$HOME/.local/share/local-labs-host/server.py"
elif [[ -f "$HOME/.local/share/3090-server/server.py" ]]; then
  install -m 0755 "$ROOT/local-labs-host/server.py" "$HOME/.local/share/3090-server/server.py"
fi
