...
async function browserPlayIndex(index){const queue=browserMedia.queue||[];if(!queue.length)throw Error('browser queue is empty');const generation=++browserPlayGeneration;index=Math.max(0,Math.min(Number(index)||0,queue.length-1));browserMedia.index=index;const entry=queue[index]||{};const hint=String(entry?.playability?.status||'');if(hint==='protected_or_restricted')throw Error('protected/restricted media · choose another item or a native output');const itemId=String(entry.id||entry.digest||'');const digest=String(entry.digest||'');const next=browserIsVideo(entry)?browserVideo:browserAudio;if(browserElement!==next)stopBrowserElement(browserElement);browserElement=next;browserMedia.state='loading';browserMedia.active=false;renderMedia(browserSnapshot(),true);browserElement.src=digest?`/api/artifact?node=${encodeURIComponent(browserMedia.sourceNode)}&digest=${encodeURIComponent(digest)}&t=${Date.now()}`:`/api/media/audio?node=${encodeURIComponent(browserMedia.sourceNode)}&id=${encodeURIComponent(itemId)}&index=${index}&t=${Date.now()}`;browserElement.load();try{await browserElement.play()}catch(err){if(generation!==browserPlayGeneration||err?.name==='AbortError'){event('MEDIA · PLAY SUPERSEDED');return false}const code=Number(browserElement.error?.code||0),detail=code===1?'aborted':code===2?'network/source':code===3?'decode':code===4?'unsupported source/codec':(err?.name||'playback');const advice=code===3||code===4?' · try another item or a native output':'';throw Error(`${detail} · ${err?.message||'media play failed'}${advice}`)}if(generation!==browserPlayGeneration)return false;browserMedia.active=true;browserMedia.state='playing';renderMedia(browserSnapshot(),true);return true}
