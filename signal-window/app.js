...
async function selectMediaOutput(next,selectEl){const previous=browserMedia.active?'__browser__':mediaNode;if(!next||next===previous)return;try{
 if(next==='__browser__'){
  const d=lastMediaState;if(!d?.queue?.length)throw Error('no media queue to hand off');browserMedia={active:false,sourceNode:String(d.node||mediaNode||''),index:Number(d.index||0),queue:d.queue.slice(),state:'loading',position:0,duration:0};event(`MEDIA · MOVE ${browserMedia.sourceNode} → ${thisDeviceLabel()}`,true);await browserPlayIndex(browserMedia.index);await fetch('/api/media/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'stop',node:browserMedia.sourceNode})});mediaDismissed=false;event(`MEDIA · MOVED → ${thisDeviceLabel()}`);renderMedia(browserSnapshot(),true);return
 }
 ...
 }catch(err){if(selectEl)selectEl.value=previous;event('MEDIA MOVE ERROR · '+err.message)}}
