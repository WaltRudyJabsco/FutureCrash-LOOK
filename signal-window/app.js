...
async function selectMediaOutput(next,selectEl){const previous=selectedMediaOutput;if(!next||next===previous)return;const sourceState=nodeMediaState;
 ...
 }catch(err){mediaHandoff=false;selectedMediaOutput=previous;if(next==='browser'){browserAudio.pause();browserMedia.active=false;browserMedia.state='stopped'}if(selectEl)selectEl.value=previous;event('MEDIA MOVE ERROR · '+err.message);renderMedia(previous==='browser'?browserSnapshot():(nodeMediaState||lastMediaState),true)}}
function renderMedia(d,force=false){
 ...
 const outputRow=document.createElement('div');outputRow.className='media-output-row';const outLabel=document.createElement('span');outLabel.textContent='OUT';const out=document.createElement('select');out.className='media-output-select';out.title='Playback output';const self=document.createElement('option');self.value='browser';self.textContent=thisDeviceLabel()+(selectedMediaOutput==='browser'?' ●':'');out.append(self);for(const row of mediaOutputs){const o=document.createElement('option');o.value=nodeTarget(row.node);const reason=String(row.reason||row.error||'').trim();o.textContent=String(row.node||'local')+(selectedMediaOutput===nodeTarget(row.node)?' ●':'')+(row.available===false&&reason?` · ${reason}`:'');o.disabled=row.available===false;out.append(o)}out.value=selectedMediaOutput||nodeTarget(d.node||mediaNode||'');
 const pickerBegin=()=>{mediaPickerActive=true};
 out.addEventListener('pointerdown',pickerBegin,{passive:true});out.addEventListener('touchstart',pickerBegin,{passive:true});out.addEventListener('focus',pickerBegin);
 out.onchange=()=>{const next=out.value;mediaPickerActive=false;selectMediaOutput(next,out)};
 out.onblur=()=>{if(!mediaPickerActive)return;mediaPickerActive=false;renderMedia(selectedMediaOutput==='browser'?browserSnapshot():(nodeMediaState||lastMediaState),true)};
 outputRow.append(outLabel,out);
