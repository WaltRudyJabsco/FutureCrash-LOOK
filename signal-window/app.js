...
let browserElement=browserAudio;
...
async function browserPlayIndex(index){const queue=browserMedia.queue||[];if(!queue.length)throw Error('browser queue is empty');index=Math.max(0,Math.min(Number(index)||0,queue.length-1));browserMedia.index=index;const entry=queue[index]||{};const itemId=String(entry.id||entry.digest||'');const digest=String(entry.digest||'');const next=browserIsVideo(entry)?browserVideo:browserAudio;if(browserElement!==next)stopBrowserElement(browserElement);browserElement=next;browserElement.src=digest?`/api/artifact?node=${encodeURIComponent(browserMedia.sourceNode)}&digest=${encodeURIComponent(digest)}&t=${Date.now()}`:`/api/media/audio?node=${encodeURIComponent(browserMedia.sourceNode)}&id=${encodeURIComponent(itemId)}&index=${index}&t=${Date.now()}`;browserElement.load();await browserElement.play();browserMedia.active=true;browserMedia.state='playing';renderMedia(browserSnapshot(),true)}
