...
async function pollMediaOutputs(){try{const r=await fetch('/api/media/outputs',{cache:'no-store'});if(!r.ok)return;const d=await r.json();mediaOutputs=Array.isArray(d.outputs)?d.outputs:[];if(!mediaNode||!mediaOutputs.some(x=>String(x.node||'')===mediaNode)){const preferred=mediaOutputs.find(x=>x.available!==false)||mediaOutputs[0];mediaNode=String(preferred?.node||'');if(mediaNode)localStorage.setItem('signal.media.node',mediaNode)}}catch{}}
...
async function selectMediaOutput(next,selectEl){const previous=browserMedia.active?'__browser__':mediaNode;if(!next||next===previous)return;try{
 if(next==='__browser__'){
  const d=lastMediaState;if(!d?.queue?.length)throw Error('no media queue to hand off');browserMedia={active:true,sourceNode:String(d.node||mediaNode||''),index:Number(d.index||0),queue:d.queue.slice(),state:'loading',position:0,duration:0};mediaDismissed=false;renderMedia(browserSnapshot(),true);event(`MEDIA · MOVE ${browserMedia.sourceNode} → ${thisDeviceLabel()}`,true);await browserPlayIndex(browserMedia.index);await fetch('/api/media/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'stop',node:browserMedia.sourceNode})});mediaDismissed=false;event(`MEDIA · MOVED → ${thisDeviceLabel()}`);renderMedia(browserSnapshot(),true);return
 }
 if(browserMedia.active){event(`MEDIA · MOVE ${thisDeviceLabel()} → ${next}`,true);browserAudio.pause();const r=await fetch('/api/media/move',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:browserMedia.sourceNode,target:next,index:browserMedia.index})});const d=await r.json();if(!r.ok||d.error)throw Error(d.error||r.statusText);browserMedia.active=false;browserMedia.state='stopped';mediaNode=next;localStorage.setItem('signal.media.node',mediaNode);lastMediaState=d;event(`MEDIA · MOVED → ${next}`);renderMedia(d,true);return
 }
 if(lastMediaState?.active&&String(lastMediaState.node||mediaNode)===mediaNode){event(`MEDIA · MOVE ${mediaNode} → ${next}`,true);const r=await fetch('/api/media/move',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:mediaNode,target:next,index:lastMediaState.index})});const d=await r.json();if(!r.ok||d.error)throw Error(d.error||r.statusText);lastMediaState=d;event(`MEDIA · MOVED → ${next}`)}mediaNode=next;localStorage.setItem('signal.media.node',mediaNode);mediaDismissed=false;await pollMedia(true)
 }catch(err){if(next==='__browser__'&&browserMedia.state==='loading'){browserMedia.active=false;browserMedia.state='stopped'}if(selectEl)selectEl.value=previous;event('MEDIA MOVE ERROR · '+err.message);if(lastMediaState)renderMedia(lastMediaState,true)}}
function renderMedia(d,force=false){
 lastMediaState=d||null;const queue=Array.isArray(d?.queue)?d.queue:[],active=Boolean(d?.active),entry=d?.entry||{};const key=[browserMedia.active?'browser':d?.node,d?.state,d?.index,d?.count,entry.artist,entry.album,entry.title].join('|');if(key!==lastMediaKey){if(active)mediaDismissed=false;lastMediaKey=key}
 ...
 const outputRow=document.createElement('div');outputRow.className='media-output-row';const outLabel=document.createElement('span');outLabel.textContent='OUT';const out=document.createElement('select');out.className='media-output-select';out.title='Playback output';const self=document.createElement('option');self.value='__browser__';self.textContent=thisDeviceLabel()+(browserMedia.active?' ●':'');out.append(self);for(const row of mediaOutputs){const o=document.createElement('option');o.value=String(row.node||'');const reason=String(row.reason||row.error||'').trim();o.textContent=String(row.node||'local')+(row.active?' ●':'')+(row.available===false&&reason?` · ${reason}`:'');o.disabled=row.available===false;out.append(o)}out.value=browserMedia.active?'__browser__':String(d.node||mediaNode||'');out.onchange=()=>selectMediaOutput(out.value,out);outputRow.append(outLabel,out);
 const meta=document.createElement('div');meta.className='media-meta';meta.textContent=`${browserMedia.active?thisDeviceLabel():(d.node||mediaNode||'local')} · queue ${d.count||queue.length} tracks · ${Number(d.index||0)+1}/${d.count||queue.length}`;
 const actions=document.createElement('div');actions.className='media-actions';const q=document.createElement('button');q.type='button';q.className='media-link';q.textContent=mediaExpanded?'Hide queue':'Queue';q.onclick=()=>{mediaExpanded=!mediaExpanded;renderMedia(browserMedia.active?browserSnapshot():d,true)};const stop=mediaButton('Stop','Stop playback','stop');stop.classList.add('media-link');const dismiss=document.createElement('button');dismiss.type='button';dismiss.className='media-link';dismiss.textContent='Dismiss';dismiss.onclick=()=>{mediaDismissed=true;mediaPanel.hidden=true};actions.append(q,stop,dismiss);
...
async function mediaControl(action,index=null){try{event('MEDIA · '+action.toUpperCase(),true);if(browserMedia.active){if(action==='toggle'){if(browserAudio.paused){await browserAudio.play();browserMedia.state='playing'}else{browserAudio.pause();browserMedia.state='paused'}}else if(action==='next'){await browserPlayIndex(browserMedia.index+1)}else if(action==='prev'){await browserPlayIndex(browserMedia.index-1)}else if(action==='jump'){await browserPlayIndex(index)}else if(action==='stop'){browserAudio.pause();browserMedia.active=false;browserMedia.state='stopped';mediaPanel.hidden=true}else throw Error('unsupported browser media control');if(browserMedia.active)renderMedia(browserSnapshot(),true);event('MEDIA · '+action.toUpperCase());return}const r=await fetch('/api/media/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,index,node:mediaNode})});const d=await r.json();if(!r.ok||d.error)throw Error(d.error||r.statusText);mediaDismissed=false;renderMedia(d,true);event('MEDIA · '+action.toUpperCase())}catch(err){event('MEDIA ERROR · '+err.message)}}
async function pollMedia(force=false){if(browserMedia.active){renderMedia(browserSnapshot(),force);return}try{const r=await fetch('/api/media?node='+encodeURIComponent(mediaNode||''),{cache:'no-store'});if(!r.ok)return;renderMedia(await r.json(),force)}catch{}}
browserAudio.addEventListener('timeupdate',()=>{if(browserMedia.active)renderMedia(browserSnapshot())});browserAudio.addEventListener('pause',()=>{if(browserMedia.active){browserMedia.state='paused';renderMedia(browserSnapshot())}});browserAudio.addEventListener('play',()=>{if(browserMedia.active){browserMedia.state='playing';renderMedia(browserSnapshot())}});browserAudio.addEventListener('ended',()=>{if(browserMedia.active&&browserMedia.index+1<browserMedia.queue.length)browserPlayIndex(browserMedia.index+1).catch(err=>event('MEDIA ERROR · '+err.message));else{browserMedia.state='stopped';browserMedia.active=false;mediaPanel.hidden=true}});
