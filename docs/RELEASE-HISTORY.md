...
- Exact paths remain exact; single unresolved LK targets may resolve through the local catalog.
- Quoted phrases opt into phrase resolution without weakening unquoted CLI grammar.
- `open`, `preview`, and `reveal` share the resolver and surface ambiguity instead of guessing.
