...
echo "Future Crash + LOOK 6.1.12 · Endpoint-Local Media + Version Repair"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.1.12" ]] || { echo "BUNDLE ERROR: expected release 6.1.12, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.1.12 with Fabric-wide endpoint management, Tailcat direct TLS transport, enforced Fabric peer authorization, accountless browser endpoint pairing, Fabric SearXNG discovery/search, Decision Plane, Content Search, Media, Artifacts, Memory, and Signal Window 1.8.1"
...
# Verify the live daemon, not merely the files on disk. An unmanaged older node can
# survive an upgrade, keep :7332, and make a simple readiness probe succeed while
# the freshly installed systemd/launchd service fails to bind. If the listener is
# one of our own stale node processes, retire it once and restart the managed unit.
live_node_version() {
...
restart_managed_node() {
  if [[ "$(uname -s)" == "Linux" ]] && command -v systemctl >/dev/null 2>&1; then
    systemctl --user restart future-crash-look-node.service >/dev/null 2>&1 || true
...
if [[ -n "$CURRENT_NODE_VERSION" && "$CURRENT_NODE_VERSION" != "$EXPECTED_RELEASE" ]]; then
  echo "  stale live node: $CURRENT_NODE_VERSION on :7332 · installed release: $EXPECTED_RELEASE"
  STALE_RETIRED=0
  if command -v lsof >/dev/null 2>&1; then
    ...
    done < <(lsof -nP -tiTCP:7332 -sTCP:LISTEN 2>/dev/null || true)
  fi
  if (( STALE_RETIRED )); then
    echo "  retired stale Future Crash node listener · restarting managed node"
    restart_managed_node
  else
    echo "INSTALL ERROR: :7332 is serving stale node $CURRENT_NODE_VERSION, but installer cannot safely retire its listener" >&2
