...
- Distributed pulse/reconciliation layer, richer leases, peer advertisements, model capability discovery and resident-model self-qualification.
- Repairs Signal→node activity wiring, LOOK/LO discovery, node identity, service restart and :7332 publication.
- Adds `lk fabric` / `fcl-node fabric` operational views.

## 3.3.10 — Inference Cancellation Reliability

- Enforces the documented 180-second timeout on primary streamed Ollama chat requests.
- Emits truthful cancellation events in machine mode.
- One-shot machine cancellation exits instead of returning to an interactive prompt.
