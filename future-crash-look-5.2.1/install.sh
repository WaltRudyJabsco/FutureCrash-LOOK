...
echo "Future Crash + LOOK 5.2.1 · Evidence Labels + Ask Flow"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "5.2.1" ]] || { echo "BUNDLE ERROR: expected release 5.2.1, found $EXPECTED_RELEASE"; exit 4; }
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 5.2.1, Persona Registry, Fabric Memory, and Signal Window 1.1.2"
