...
def peer_rows():
    ...
    for peer in (d.get("Peer") or {}).values():
        ...
        result.append({"name": peer.get("HostName") or dns or (ips[0] if ips else "peer"),
                       "dns": dns, "ips": ips, "online": bool(peer.get("Online"))})
...
def print_fabric():
    ad = advertisement()
    print(f"FUTURE CRASH FABRIC · pulse {ad['pulse']['number']} · node {ad['identity']['name']}")
    print("─" * 72)
    active = ad["supervisor"]["active"]
    if active:
        print(f"local   {active['priority']:<11} {active['phase']:<14} {active['owner']} · {active['state']}")
    else:
        print("local   idle")
    models = ad["inference"]["models"]
    if models:
        print("models  " + ", ".join(m["name"] + ("*" if m.get("resident") else "") for m in models))
    for p in PEERS.public():
        if p.get("node"):
            n = p["node"]
            a = (n.get("supervisor") or {}).get("active")
            state = (a or {}).get("phase") or "idle"
            print(f"peer    {p['name']:<18} {state:<14} pulse {(n.get('pulse') or {}).get('number','?')}")
        else:
            print(f"peer    {p['name']:<18} {'online' if p['online'] else 'offline':<14} no node advertisement")
...
def main():
    ...
    a = ap.parse_args()
    if a.command == "status":
        print(json.dumps(node_info(), indent=2)); return
    if a.command == "nodes":
        PEERS.refresh(); print(json.dumps({"self": node_info(), "peers": PEERS.public()}, indent=2)); return
    if a.command == "activity":
        print(json.dumps(SUP.status(), indent=2)); return
    if a.command == "pulse":
        print(json.dumps({"epoch": "unix-1s-v1", "number": pulse_number(), "period_ms": int(PULSE_SECONDS*1000)}, indent=2)); return
    if a.command == "models":
        print(json.dumps({"models": MODELS.discover(force=True)}, indent=2)); return
    if a.command == "qualify":
        if not a.target:
            ap.error("qualify requires a model name")
        print(json.dumps(qualify_model(a.target), indent=2)); return
    if a.command == "fabric":
        PEERS.refresh(); print_fabric(); return
