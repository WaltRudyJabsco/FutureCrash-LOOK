...
echo "Future Crash + LOOK 6.7.6 · CANONICAL EDGE"
echo "────────────────────────────────────────"
...
[[ "$EXPECTED_RELEASE" == "6.7.6" ]] || { echo "BUNDLE ERROR: expected release 6.7.6, found $EXPECTED_RELEASE"; exit 4; }
echo "BUNDLE SOURCE  $ROOT"
echo "BUNDLE RELEASE $EXPECTED_RELEASE · CANONICAL EDGE"
...
if ((DRY_RUN)); then
  echo
  echo "[dry-run] would install/restart Unified Node 6.7.6 · CANONICAL EDGE with signed Fabric rendezvous, Tailcat direct transport, ONE BRAIN cognition, LIVING MIND memory, browser endpoints, SearXNG, Media, Artifacts, and Signal Window 1.10.5"
